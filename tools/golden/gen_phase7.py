...
# A WATTVAR run whose curve demands the FULL headroom (y=-1.0 at high output), so
# `Check_Qlimits_WV` hits the kvar limit (`FWVOperation = 0.2`) and `Calc_PQ_WV`
# takes the `PLimitEndpu := wattvar_curve.GetXValue(QDesireEndpu)` branch — the
# kvar-limited path the other wattvar goldens (max |y|=0.8) never reach offline (it
# is otherwise covered only live by the corpus `SnapShot_wattvar`). Pins the
# converged (P, Q) of that branch against the oracle.
def deck_invcontrol_wattvar_qlim() -> list[str]:
    return [
        "new circuit.t basekv=12.47 phases=3 bus1=src basefreq=60",
        "new Line.l1 bus1=src bus2=b phases=3 r1=0.1 x1=0.3 c1=0 length=1 units=km",
        "new XYcurve.wv npts=4 yarray=(0.1 0.1 -1 -1) xarray=(0 0.25 0.75 1.0)",
        "new PVSystem.pv bus1=b phases=3 kV=12.47 kVA=1200 Pmpp=1000 pf=1.0 "
        "irradiance=1.0",
        "new InvControl.ic mode=WATTVAR wattvar_curve=wv RefReactivePower=VARMAX",
        *PV_TAIL,
