...
# tests/_settings.py, branch fastdss (0.16.0b2), BSD-3-Clause — see LICENSE.
# Local modifications marked "# dss-rs:". Run with the Oddie venv
# (tools/opendss/.venv); see README.md in this directory.
...
if USE_ODDIE is not None and USE_ODDIE.upper() not in ('0', 'FALSE', 'F', 'OFF', 'NO'):
    ...
    if USE_ODDIE.startswith('oddie:'):
        # dss-rs: repo-native engine spec `oddie:r3723|r4088|r4133` — resolve
        # the vendored EPRI DLL via tools/opendss/revisions.json and verify the
        # loaded engine version, mirroring oracle_server.make_engine (a wrong
        # DLL must fail loudly, never silently compare the wrong engine).
        import json
        _rev = USE_ODDIE.split(':', 1)[1]
        _opendss_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _repo = os.path.dirname(os.path.dirname(_opendss_dir))
        with open(os.path.join(_opendss_dir, 'revisions.json')) as _f:
            _revs = json.load(_f)
        if _rev not in _revs:
            sys.exit(f"unknown revision {_rev!r} (revisions.json: {sorted(_revs)})")
        _expect = _revs[_rev].get('expect_version', '')
        if not _expect:
            sys.exit(f"revisions.json entry {_rev!r} has an empty expect_version "
                     "— run tools/opendss/smoke.py first")
        _dll = os.path.join(_repo, *_revs[_rev]['dll'].split('/'))
        DSS = IOddieDSS(_dll)
        if _expect not in DSS.Version:
            sys.exit(f"loaded engine {DSS.Version!r} does not contain expected "
                     f"{_expect!r} for {_rev}")
    elif USE_ODDIE != '1':
...
# dss-rs: anchor the corpus to the repo's vendored copy (same electricdss-tst
# layout upstream clones); DSS_RS_TST_DIR overrides. Upstream's ZIP_FN
# (tests/data/13Bus.zip) is dropped — unused by save_outputs/compare_outputs.
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
BASE_DIR = os.environ.get(
    'DSS_RS_TST_DIR',
    os.path.join(_REPO_ROOT, 'tests', 'corpus', 'electricdss-tst'),
)
