...
# tests/save_outputs.py, branch fastdss (0.16.0b2), BSD-3-Clause — see LICENSE.
# Local modifications marked "# dss-rs:": corpus anchored to the repo's
# vendored copy, COM branch dropped, editor suppression hardened, per-case
# CorpusGuard so the vendored corpus stays pristine, results zip -> tmp/.
# Run with the Oddie venv; see README.md in this directory.
...
try:
    from ._settings import test_filenames, cimxml_test_filenames, USE_ODDIE, BASE_DIR
except ImportError:
    from _settings import test_filenames, cimxml_test_filenames, USE_ODDIE, BASE_DIR

# dss-rs: the corpus-hygiene guard shared with the oracle server (restores the
# case dir after each run; NOT recursive — end sweeps with
# `git status tests/corpus`).
_TOOLS_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
REPO_ROOT = os.path.dirname(_TOOLS_DIR)
sys.path.insert(0, os.path.join(_TOOLS_DIR, 'oracle'))
            ...
            for field in fields:
                ...
                try:
                    ...
                    if 'not implemented' in e.args[1].lower():
                        #print(e.args)
                        continue
                    # dss-rs: older official DLLs (r3723 = 9.8) lack API
                    # exports the fastdss wrapper knows (e.g.
                    # Transformers_Get_LossesByType -> "(Oddie) Variant array
                    # error..."). Skip the field, keep the case — the whole
                    # point of an old-revision sweep is inventorying what
                    # exists. Never triggers for the dss-extensions engine.
                    if '(oddie)' in e.args[1].lower():
                        continue
                    raise
                except StopIteration:
...
def save_state(dss: dss.IDSS, runtime: float = 0.0) -> str:
    ...
    for key, dss_api_cls in dss_classes.items():
        ...
        try:
            ...
            # dss-rs: whole collections can be absent from older official
            # DLLs (r3723: "(Oddie) Function StoragesI was not found...",
            # raised from the collection iterator itself). Skip the
            # collection, keep the case. Never triggers for dss-extensions.
            if '(oddie)' in e.args[1].lower():
                print(f'[COLLECTION SKIPPED] {key}: {e.args[1]}')
                continue
            raise
...
if __name__ == '__main__':
    ...
    # dss-rs: the vendored corpus (or DSS_RS_TST_DIR), resolved in _settings.
    ROOT_DIR = BASE_DIR
    ...
    if SAVE_DSSX_OUTPUT_ODD:
        ...
        # dss-rs: the upstream COM branch (comtypes + machine-global regsvr32
        # registration) is dropped — this repo drives EPRI engines exclusively
        # through the vendored Oddie DLLs (tools/opendss/README.md).
        sys.exit("COM engine not supported here: pass `dss-extensions` or set "
                 "DSS_EXTENSIONS_TEST_ODDIE=oddie:<rev> + `dss-extensions-odd`")
    ...
    try:
        ...
        check_error()
        # dss-rs: upstream's `true.exe` path is machine-dependent and the EPRI
        # engine persists the Editor override to the user's OpenDSS registry
        # settings on dispose. Use the proven oracle_server pair instead:
        # suppress the registry write, then a GUI-subsystem no-op editor.
        try:
            DSS.Text.Command = 'Set RegistryUpdate=No'
        except:
            pass
        DSS.Text.Command = 'set Editor=rundll32.exe'
    ...
    total_runtime = 0.0
    # dss-rs: results go to tmp/dsspy_validation/, never the CWD (the sweep
    # must not scatter zips around the repo or the corpus).
    out_dir = os.path.join(REPO_ROOT, 'tmp', 'dsspy_validation')
    os.makedirs(out_dir, exist_ok=True)
    zip_fn = f'results{suffix}.zip'
    with ZipFile(os.path.join(out_dir, zip_fn), mode='a', compression=ZIP_DEFLATED) as zip_out:
        for fn in cimxml_test_filenames + test_filenames:
            ...
            # dss-rs: guard the case dir — run products (export profile, CIM
            # XML, CSVs) are zipped inside the guard scope, then deleted /
            # restored on exit so the vendored corpus stays pristine. The
            # guard is non-recursive: subdirectory writes escape it, hence
            # the mandatory `git status tests/corpus` after a sweep.
            with CorpusGuard(fn):
                try:
                    has_closedi = False
