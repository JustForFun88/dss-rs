...
gate-green Phase-7 tip). **WP8.1–8.3 COMPLETE + audited; WP8.4 (Show) steps 1–12
...
EnergyMeter zone-tree pair) + dispatcher; ~27 `Show` reports ported).
**~5 real `Show` reports remain** (`Controlled`/`Isolated`/`Topology`/`busflow`/
`LineConstants`) + `autoadded`/`QueryLog` (headless FireOffEditor no-ops) +
`deltaV` (deferred) — all still *silent* no-ops, `TODO(WP8)`. Full Phase-8 detail
is in **§1f**; the current frontier:
...
- **WP8.4 (Show reports) — step 12 COMPLETE, gate-green** (the EnergyMeter
  ...
  was never a blocker). **next = continue WP8.4** (`Controlled`; the remaining CktTree
  families `Isolated`/`Topology`/`busflow` — need `GetTopology`/`GetIsolatedSubArea`;
  `LineConstants`).
