...
gate-green Phase-7 tip). **WP8.1–8.3 COMPLETE + audited; WP8.4 (Show) steps 1–13
...
new `CktElement::controlled_element` accessor) + dispatcher; ~28 `Show` reports
ported). **~4 real `Show` reports remain** (`Isolated`/`Topology`/`busflow`/
...
- **WP8.4 (Show reports) — step 13 COMPLETE, gate-green** (`Show Controlled`):
  ...
  **derive** the PD→controls map by scanning `ckt.controls` (creation order) and
  ...
  blocked a deck). **next = continue WP8.4** (`LineConstants` — reuses the WP7.1
  LineGeometry `z_matrix`/`yc_matrix`; then `busflow` `ShowBusPowers`; then the
...
cargo test --workspace      # dss-core lib 744, golden_feeders 1,
                            ...
                            # golden_phase7_protection 1, golden_phase8 104,
