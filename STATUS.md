...
**D34**'s unported executive commands sit at `ORPHANED_GAPS.md` §1.21. And **G1.10b** (2026-09-12, `lane-s`, `4bbc6405` + `9b07466a` + `934a4275` + `a4bbfd32`/`f93eb4ec` + docs), the run files' **CONTENTS** — nine report kinds through the SAME `ExportPolicy`
...
gaps landed ahead of it — the `InShowResults` bracket round Show/Export/Save (`DoSaveCmd`'s latch NOT reproduced, `to_opendss/72`) and nine PC `node_ref` guards (`to_opendss/74`). The lanes' micro decks met here: corpus **526** cases / 522 live;
...
**Next.** **G1.10b** is complete on `lane-s` (2026-09-12, the five commits above — F0 + F2b + surface, then two settlement rounds: 32 findings 28 fixed / 4 recorded, then 5 fixed — plus this docs commit) and awaits the landing into `update`; **G1.10c** runs on `lane-e`
