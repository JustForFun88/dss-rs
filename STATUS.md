...
(`pick_a_fault → None`). `set steptime`/`processtime`/`totaltime` are now
silent no-ops (Pascal has no Set arm for 106/108; 107 is a wall-clock
accumulator not modeled) — removes the spurious "not ported yet" divergence.
GFM/GFL_IEEE123 skip-notes retagged (Plot is a headless no-op, steptime/
dynamics-GFM ported; they stay deferred purely as a full-feeder-trajectory
...
gate-green Phase-7 tip). **WP8.1–8.7 COMPLETE + audited** (WP8.5 steps 1–6
