...
/// PDElement with `NumAmpRatings > 1`, when the global season index is in
/// `[0, NumAmpRatings)`; both norm and emerg take the same seasonal value.
/// Feature-sensitive: the `-1`/out-of-range guard must fall back to the base
/// ratings (a regression that dropped the guard would index the array).
#[test]
fn get_ratings_applies_seasonal_index() {
    ...
    // Pascal guard is `idx < NumAmpRatings` (not `> 1`): a single-rating element
    // at idx 0 still applies `AmpRatings[0]` (a no-op when `amp_ratings[0]` ==
    // `norm_amps`, as it is for a normally-initialized element). Any idx >= 1 is
    // out of range → base ratings.
