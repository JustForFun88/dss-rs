...
/// no-op, ExecOptions.pas l.755-758); `set totaltime` writes a diagnostic
/// accumulator with zero solution effect. This port models no wall-clock timing,
/// so all three are silent no-ops — they must NOT error (the old "not ported yet"
/// divergence) and must NOT change the solution. Confirmed against the pinned
/// oracle (`set steptime`/`processtime`/`totaltime` raise no error, solution
/// unchanged).
#[test]
