...
fn export_records_scoped_not_ported() {
    ...
    // (2) A still-unported export records the scoped `NOT_PORTED`. `EventLog`(33)
    //     is not solution-guarded (33 ∉ the #24712 ptr set), so no solve is
    //     needed; it lands in WP8.3. (WP8.2 step 3 ported the matrix/summary
    //     exports, so `Summary`/`Y`/`Yprims`/`SeqZ`/`Result` are now real reports,
    //     gated by `golden_phase8.rs`, not this NOT_PORTED test.)
    let mut dss = Dss::new();
    dss.command("new circuit.t basekv=12.47 phases=3 bus1=src");
    dss.command(&set_dp);
    dss.command("export eventlog");
    assert_eq!(dss.errors().len(), 1, "{:?}", dss.errors());
    assert!(
        dss.errors()[0].contains("\"EventLog\"") && dss.errors()[0].contains("not ported"),
        "{:?}",
        dss.errors()
    );
