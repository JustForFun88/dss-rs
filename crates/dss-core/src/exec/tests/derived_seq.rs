...
/// / `modes/makeposseq/makeposseq_gic.dss` are in-repo synthetic decks that
/// write no file, so no directory guard is needed. Never `.inputs/` — the
/// corpus tree is the one the live gate reads (CLAUDE.md).
...
// Expected-value pin — coordinator decision **D31** (2026-09-05), taken when
// G1.3b landed on `update`: `modes/makeposseq/makeposseq_gic.dss` is the ONE
// corpus case that puts the 1φ-positive-sequence arm on the `r4133` channel.
// `update` had just split the GICTransformer coverage out of
// `makeposseq_shunt.dss` into that r4133-gated deck (coordinator decisions
// D12/D14 — the pinned capi 0.14.5 oracle is nondeterministic on any deck that
// instantiates a `GICTransformer`), and the deck's `solve → makeposseq → solve`
// tail leaves all four of its elements 1-phase in a positive-sequence circuit.
// So r4133 reports every one of their powers one slot late, which is the defect
// the pin above states on a synthetic fixture — here it is stated on the gated
// deck itself, against numbers read from the EPRI r4133 DLL on this very deck,
// so that the ledger entry `r4133-posseq-seqpowers-slot-gic` (which excludes
// `seq_powers` there, and nothing else) rests on both numbers rather than on a
// masked comparison. `SeqCurrents` and `SeqVoltages` are correct on r4133 here
// — they go through `Calc*`, not through mode 9 — and stay oracle-compared
// live; asserting them below against the same probe is what makes the
// exclusion's `seq_powers`-only scope honest.
/// Every terminal power of the r4133-gated `makeposseq_gic` deck sits in that
/// terminal's positive-sequence slot `3t+1`, where r4133 writes it at `t+2`.
#[test]
fn the_gic_posseq_deck_keeps_every_terminal_power_in_its_positive_slot() {
    let mut dss = compile_corpus_deck("modes/makeposseq/makeposseq_gic.dss");
    let snaps = dss.snapshot_elements();

    // `(element, per-terminal (|V+|, |I+|, S+.re, S+.im))` exactly as the EPRI
    // r4133 DLL (`Version 11.0.0.1`, rev `r4133`) reported them on this deck
    // through `crates/dss-epri` on 2026-09-05: the two magnitudes at the correct
    // slot `3t+1`, the power of terminal `t` at flat slot `t+2` —
    // `Vsource.source`'s `[0, 0, −14353.24, 0, 0, 0]` against the port's
    // `[0, −14353.24, 0, 0, 0, 0]`, and `Line.feed`'s two terminals in slots
    // `2` and `3` against the port's `1` and `4`.
    /// One terminal's oracle row: `(|V+|, |I+|, S+.re, S+.im)`.
    type Terminal = (f64, f64, f64, f64);
    const R4133: &[(&str, &[Terminal])] = &[
        (
            "Vsource.source",
            &[
                (
        ...
        (
            ...
            &[
                ...
                (0.0, 3991.5749437423383, 0.0, 0.0),
            ],
        ),
        (
            "Load.ld1",
    ...
    assert_eq!(
        snaps.len(),
        R4133.len(),
        "the deck's element set moved: this pin carries one oracle row per \
         element and every one of them is on the 1-phase positive-sequence arm",
    );

    for (name, terms) in R4133 {
        let e = elem(&snaps, name);
        assert_eq!(e.n_phases, 1, "{name} is 1-phase after `makeposseq`");
        assert_eq!(
            e.seq_arm,
            SeqArm::PosSeqSinglePhase,
            "{name} takes the 1-phase positive-sequence arm — the arm this deck \
             brings onto the r4133 channel",
        );
        assert_eq!(e.n_terms, terms.len(), "{name} terminals");
        assert_eq!(
            e.seq_powers.len(),
            3 * terms.len(),
            "{name} SeqPowers slots"
        );

        for (t, &(v012, i012, kw, kvar)) in terms.iter().enumerate() {
            let slot = 3 * t + 1;
            let bv = MICRO_V_ABS + MICRO_V_REL * v012;
            let bi = MICRO_I_ABS + MICRO_I_REL * i012;
            // The two magnitudes r4133 gets right, in the slot both engines use.
            assert!(
                (e.seq_voltages[slot] - v012).abs() <= bv,
                "{name} terminal {} |V+| = {}; r4133 reads {v012} V, band {bv}",
                t + 1,
                e.seq_voltages[slot],
            );
            assert!(
                (e.seq_currents[slot] - i012).abs() <= bi,
                "{name} terminal {} |I+| = {}; r4133 reads {i012} A, band {bi}",
                t + 1,
                e.seq_currents[slot],
            );
            // …and the power, which the port puts one slot earlier than r4133.
            let band = power_band(bv, bi, v012, i012);
            assert!(
                (e.seq_powers[slot] - Complex64::new(kw, kvar)).norm() <= band,
                "{name} terminal {} S+ = {} kVA in slot {slot}; r4133 reports \
                 {kw} + j{kvar} kVA in slot {} instead \
                 (DCktElement.pas:760 `Count := 2`, `:768` `inc(count)`), band {band}",
                t + 1,
                e.seq_powers[slot],
                t + 2,
            );
        ...
        // Every slot r4133 walks into is EXACTLY zero here — the port writes one
        // slot per terminal into an initialized result, so this is an equality.
        // It is also what the live gate reds on: `seq_powers[1]` non-zero where
        // r4133 has 0, hence the ledger entry and this pin.
        for slot in (0..e.seq_powers.len()).filter(|s| s % 3 != 1) {
            assert_eq!(
                e.seq_powers[slot],
                Complex64::ZERO,
                "{name} slot {slot} must be exactly zero — r4133 fills slots \
                 2..{} instead of 1, 4, …",
                terms.len() + 1,
            );
        }
