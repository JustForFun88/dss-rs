...
(`crates/dss-core/tests/corpus_gate/ledger.rs:1091`) — the same caps
...
`CktElement.Yprim` live on both channels (`tools/oracle/oracle_server.py:1126`,
`crates/dss-epri/src/capture.rs:669` over `Engine::element_yprim`
...
(`crates/dss-core/tests/corpus_gate.rs:497`) pins is therefore the agreement of
...
`tools/oracle/oracle_server.py:1076`, `Engine::relcalc`,
...
(`crates/dss-core/src/exec/view.rs:2144` / `:2095`) — `&self` reads, the
...
(`crates/dss-core/tests/corpus_gate/ledger.rs:658`) closes that with three
...
`BARE_CHANNELS_ALLOWED` (`ledger.rs:676`), which is **empty**. The rules are
...
`property`-scoped `ledger.json` entry (`corpus_gate/ledger.rs:1313`, `:1370`) —
