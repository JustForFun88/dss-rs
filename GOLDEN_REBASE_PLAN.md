...
> §G1.3d as a whole. **0 new ledger entries, 0 new causes** (58 / 31 unchanged), **10** measured
...
empty (`crates/dss-epri/tests/modes.rs`). **2026-09-04, G1.3a:** the table is now
**97** rows — `CktElement.Enabled` (`CktElementI(12)`, `DCktElement.pas:263`) is the
safety predicate its enabled-only polar capture needs — and the probe was re-run at
97/97 `Served`, still zero misses. The `X_Get_Y` spellings below and above
